...
import { getCategoryBySlug, getProducts } from "@/app/(store)/actions"
...
export default async function CategoryPage({
  ...
  const productsResult = await getProducts({ categoryId: category.id, limit: 48 })
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <nav className="mb-6 flex items-center gap-1.5 text-sm text-muted-foreground">
        ...
        <ChevronRight className="h-3.5 w-3.5" />
        <span className="font-medium text-foreground">{category.name_fr}</span>
      </nav>

      <div className="mb-8">
        <h1 className="text-2xl font-bold text-foreground sm:text-3xl">{category.name_fr}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{category.name_ar}</p>
        <p className="mt-2 text-sm text-muted-foreground">{products.length} produit(s)</p>
      </div>
