...
function transformProduct(dbProduct: any): Product {
  const stockCount = Number(dbProduct.stock || 0);
  return {
    id: dbProduct.id,
    slug: dbProduct.slug,
    name: {
    ...
    priceBaridiMob: dbProduct.price_baridimob_dzd,
    priceFlexy: dbProduct.price_flexy_dzd,
    priceSlickPay: dbProduct.price_slickpay_dzd,
    ...
    stockCount,
    specs: (dbProduct.product_specs || []).reduce(
    ...
    specsAr: (dbProduct.product_specs || []).reduce(
      (acc: Record<string, string>, spec: any) => {
...
export async function generateMetadata({
  ...
  const description =
    product.description_fr || `Découvrez ${title} chez Edigiya.`;
  return {
    title: `${title} | Edigiya`,
    description,
    openGraph: {
