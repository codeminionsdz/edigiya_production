    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {(order.order_items || []).some((item: any) => ['credentials', 'code'].includes(item.products?.fulfillment_type)) && order.payments?.[0]?.status === 'paid' && <p className="-mt-4 text-sm text-emerald-700">Paiement vérifié — livraison numérique automatique</p>}
          {!(order.order_items || []).some((item: any) => ['credentials', 'code'].includes(item.products?.fulfillment_type)) && <FulfillmentContentPanel order={order} onFulfillmentReady={(fulfillment, itemCount) => { setDeliveryContentCount(itemCount); if (fulfillment && !order.order_fulfillments?.[0]) setOrder((current: any) => ({ ...current, order_fulfillments: [fulfillment] })); }} />}
          <Card>
            ...
            <CardContent className="space-y-4 text-sm">
              <div className="flex items-center justify-between"><span className="text-muted-foreground">Type</span><span className="font-semibold">{(order.order_items || []).some((item: any) => ['credentials', 'code'].includes(item.products?.fulfillment_type)) ? 'Automatique' : 'Manuelle'}</span></div>
