-- Keep the existing one-click paid digital fulfillment flow reliable when an
-- active Phase 1 reservation expires before the unique-unit allocation call.
-- This does not create stock, reservations, units, or Vault secrets.
...
BEGIN
  ...
  -- The Phase 1 row remains active while payment/allocation is in progress.
  -- Extend only that existing row; do not create a new reservation.
  ...
  SELECT reservation_row.order_id,
  ...
  SELECT order_item_row.order_id,
         order_item_row.product_id,
         order_item_row.variant_id,
         order_item_row.qty
    INTO v_item
  FROM public.order_items AS order_item_row
  WHERE order_item_row.id = p_order_item_id
  FOR UPDATE;
  ...
  SELECT reservation_item_row.product_id,
         reservation_item_row.variant_id,
         reservation_item_row.quantity
  ...
  ORDER BY reservation_item_row.id
  LIMIT 1
  FOR UPDATE;
  ...
  IF v_count > 0 THEN
    ...
    END IF;
    RETURN QUERY
    SELECT digital_reservation_row.id,
           digital_reservation_row.digital_inventory_unit_id,
           digital_reservation_row.reservation_slot,
           digital_reservation_row.status
    FROM public.digital_unit_reservations AS digital_reservation_row
    WHERE digital_reservation_row.reservation_id = p_reservation_id
      AND digital_reservation_row.order_item_id = p_order_item_id
      AND digital_reservation_row.reservation_idempotency_key = p_reservation_idempotency_key
