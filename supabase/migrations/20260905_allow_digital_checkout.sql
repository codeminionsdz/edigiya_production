-- Digital checkout keeps legacy shipping columns but does not require physical delivery.
ALTER TABLE orders ALTER COLUMN wilaya_code DROP NOT NULL;
...
AS $$
DECLARE v_order_id UUID; v_payment_id UUID; v_existing_session UUID; v_item JSONB; v_product RECORD; v_variant RECORD; v_qty INTEGER; v_unit NUMERIC; v_line NUMERIC; v_subtotal NUMERIC := 0; v_shipping NUMERIC; v_total NUMERIC; v_method TEXT := p_payment_method;
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    SELECT pr.id, pr.title_fr, pr.price_dzd, pr.stock, pr.is_active INTO v_product FROM products pr WHERE pr.id = (v_item->>'product_id')::UUID;
    ...
    IF NULLIF(v_item->>'variant_id','') IS NOT NULL THEN
      SELECT pv.id, pv.product_id, pv.price_delta_dzd, pv.stock INTO v_variant FROM product_variants pv WHERE pv.id = (v_item->>'variant_id')::UUID;
  ...
  SELECT v_order_id, pr.id, pv.id, pr.title_fr, pr.price_dzd + COALESCE(pv.price_delta_dzd, 0), (item->>'qty')::INTEGER, (pr.price_dzd + COALESCE(pv.price_delta_dzd, 0)) * (item->>'qty')::INTEGER
  FROM jsonb_array_elements(p_items) item JOIN products pr ON pr.id = (item->>'product_id')::UUID LEFT JOIN product_variants pv ON pv.id = NULLIF(item->>'variant_id','')::UUID AND pv.product_id = pr.id;
  ...
  INSERT INTO payment_events(payment_id, event_type, actor_type, metadata) VALUES (v_payment_id, 'payment_created', 'customer', jsonb_build_object('amount_dzd', v_total, 'method', v_method));
